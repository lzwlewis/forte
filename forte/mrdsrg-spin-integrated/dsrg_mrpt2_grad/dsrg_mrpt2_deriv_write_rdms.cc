...
void DSRG_MRPT2::write_2rdm_spin_dependent() {
    if (ints_->integral_type() != Custom) {
        ints_->wfn();
        //throw std::runtime_error("Test for PySCF");
        ...
        // NOTICE: write spin_dependent two-RDMs coefficients using IWL
        outfile->Printf("\n    Writing 2RDM Coefficients for PySCF ............. ");
        auto psio_ = _default_psio_lib_;
        IWL d2aa(psio_.get(), PSIF_MO_AA_TPDM, 1.0e-14, 0, 0);
        IWL d2ab(psio_.get(), PSIF_MO_AB_TPDM, 1.0e-14, 0, 0);
        IWL d2bb(psio_.get(), PSIF_MO_BB_TPDM, 1.0e-14, 0, 0);

        std::shared_ptr<psi::PsiOutStream> printer_aa = std::make_shared<PsiOutStream>("d2aa.dat", std::ios_base::app);
        std::shared_ptr<psi::PsiOutStream> printer_ab = std::make_shared<PsiOutStream>("d2ab.dat", std::ios_base::app);
        std::shared_ptr<psi::PsiOutStream> printer_bb = std::make_shared<PsiOutStream>("d2bb.dat", std::ios_base::app);
        ...
        for (size_t i = 0, size_c = core_all_.size(); i < size_c; ++i) {
            ...
            for (size_t a = 0, size_v = virt_all_.size(); a < size_v; ++a) {
                ...
                for (size_t j = 0; j < size_c; ++j) {
                    ...
                    printer_aa->Printf("%d %d %d %d = %20.10f\n", m, e, m1, m1, z_a);
                    printer_bb->Printf("%d %d %d %d = %20.10f\n", m, e, m1, m1, z_b);
                    printer_aa->Printf("%d %d %d %d = %20.10f\n", m, m1, m1, e, -z_a);
                    printer_bb->Printf("%d %d %d %d = %20.10f\n", m, m1, m1, e, -z_b);
                    printer_ab->Printf("%d %d %d %d = %20.10f\n", m, e, m1, m1, 2.0 * (z_a + z_b));
                }
            }
        }
            ...
            for (size_t j = 0; j < size_c; ++j) {
                ...
                for (size_t k = 0, size_a = actv_all_.size(); k < size_a; ++k) {
                    ...
                    for (size_t l = 0; l < size_a; ++l) {
                        ...
                        printer_aa->Printf("%d %d %d %d = %20.10f\n", n, m, a1, u1, 0.5 * z_a * g_a);
                        printer_bb->Printf("%d %d %d %d = %20.10f\n", n, m, a1, u1, 0.5 * z_b * g_b);
                        printer_aa->Printf("%d %d %d %d = %20.10f\n", n, u1, a1, m, -0.5 * z_a * g_a);
                        printer_bb->Printf("%d %d %d %d = %20.10f\n", n, u1, a1, m, -0.5 * z_b * g_b);
                        printer_ab->Printf("%d %d %d %d = %20.10f\n", n, m, a1, u1, (z_a * g_b + z_b * g_a));
                    }
                }
            }
        }
            ...
            for (size_t j = 0; j < size_v; ++j) {
                ...
                for (size_t k = 0, size_a = actv_all_.size(); k < size_a; ++k) {
                    ...
                    for (size_t l = 0; l < size_a; ++l) {
                        ...
                        printer_aa->Printf("%d %d %d %d = %20.10f\n", f, e, a1, u1, 0.5 * z_a * g_a);
			printer_bb->Printf("%d %d %d %d = %20.10f\n", f, e, a1, u1, 0.5 * z_b * g_b);
                        printer_aa->Printf("%d %d %d %d = %20.10f\n", f, u1, a1, e, -0.5 * z_a * g_a);
                        printer_bb->Printf("%d %d %d %d = %20.10f\n", f, u1, a1, e, -0.5 * z_b * g_b);
                        printer_ab->Printf("%d %d %d %d = %20.10f\n", f, e, a1, u1, (z_a * g_b + z_b * g_a));
                    }
                }
            }
        }
